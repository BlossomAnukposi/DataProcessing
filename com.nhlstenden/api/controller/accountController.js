...
class AccountController extends ControllerParent {
  ...
  async createAccount(req, res) {
    ...
    try {
      ...
      // Get the full account details to generate token
      const fullAccount = await AccountModel.getAccountByEmail(email);

      // Generate token
      const token = TokenUtils.generateToken(fullAccount);

      // Remove password from response
      const { password: _, ...accountWithoutPassword } = fullAccount;
