...
class PreferenceController extends ControllerParent {
  ...
  async createPreference(req, res) {
    ...
    try {
      ...
      const profileExists = await PreferenceModel.validateProfile(profileId);
      if (!profileExists) {
        return this.sendResponse(
          res,
          404,
      ...
      // Create preference
      const preference = await PreferenceModel.createPreference(
        profileId,
        movieId,
        seriesId,
        ageClassification,
  ...
  async getPreferencesByProfile(req, res) {
    ...
    try {
      ...
      this.sendResponse(
        res,
        200,
        "fetched preferences successfully",
        result,
        isXml
      );
    } catch (err) {
