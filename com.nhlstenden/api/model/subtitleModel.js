...
class SubtitleModel extends ModelParent {
    ...
    async updateSubtitleById(subtitleId, language, content, movieId = null, episodeId = null) {
        try {
            const result = await database.query(
                `SELECT * FROM public.update_subtitle($1, $2, $3, $4, $5)`,
                [subtitleId, language, content, movieId, episodeId]
            );

            if (!result || result.length === 0) {
                throw new Error("Failed to update subtitle. Subtitle not found or invalid data.");
            }
